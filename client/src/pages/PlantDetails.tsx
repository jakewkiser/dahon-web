...
import { db, deletePlant, Plant, listCareLogs, CareLog, addCareLog, HAS_STORAGE, uploadFileAndGetURL } from '../lib/firebase'
...
export default function PlantDetails() {
  ...
  const [d, setD] = useState('') // date string
  ...
  useEffect(()=>{
    async function load() {
      ...
      const latest = await listCareLogs(id, 20) // grab a few more for better context
      setLogs(latest)
      setLoading(false)
    }
    load()
  ...
  async function addLog(e: React.FormEvent) {
    ...
    if (!id || !user) return
    if (!d) { setError('Care log date is required.'); return }
    setSaving(true)
    try {
    ...
    <Card className="max-w-2xl">
      <div className="flex items-start justify-between gap-3">
        <div>
          ...
          {(plant as any).location && <div className="opacity-80">Location: {(plant as any).location}</div>}
          {plant.lastCareAt && <div className="text-sm opacity-70 mt-1">Last cared: {fmt.format(new Date(plant.lastCareAt))}</div>}
        </div>
        ...
        {logs.map(l => (
          <div key={l.id} className="text-sm flex items-center justify-between gap-3">
            <div className="flex items-center gap-3">
              {/* Thumbnail opens lightbox; never obstructs details since it’s modal */}
              {l.photoUrl && (
                <button
      ...
      <div className="space-y-2">
        ...
        {error && <div className="text-sm text-red-600 bg-red-50 dark:bg-red-950/40 rounded-xl p-2">{error}</div>}
        {notice && <div className="text-sm text-amber-700 bg-amber-50 dark:bg-amber-900/40 rounded-xl p-2">{notice}</div>}
        <form onSubmit={addLog} className="grid sm:grid-cols-2 gap-3">
          <div>
            <label className="text-sm opacity-70">Type</label>
            <select className="glass w-full px-3 py-2 rounded-xl" value={t} onChange={e=> setT(e.target.value as any)}>
          ...
          <div>
            ...
            <input className="glass w-full px-3 py-2 rounded-xl" type="date" value={d} onChange={e=> setD(e.target.value)} />
          </div>
          ...
          <div className="sm:col-span-2">
            <Button className="text-ink" disabled={saving} type="submit">{saving ? 'Saving…' : 'Add log'}</Button>
          </div>
        </form>
      </div>
